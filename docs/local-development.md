...
| `NABA_HDWP_EMBED_CDN_HOST` | Repoints the `<nova-stats-*>` `embed` build script/style tags at a locally-served build instead of the production CDN (`https://cdn.statistics.stream`) — see below. |
...
2. **Set the defines** in a must-use plugin
   ...
   define('NABA_HDWP_EMBED_CDN_HOST', 'https://opendxp-nova-stats.ddev.site');
...
the `embed` build from `NABA_HDWP_EMBED_CDN_HOST` (or the production CDN when that
...
4. **WordPress:** `cd env-wordpress && ddev start` (mount + defines active, including
   `NABA_HDWP_EMBED_CDN_HOST`). Activate the plugin in wp-admin if needed.
